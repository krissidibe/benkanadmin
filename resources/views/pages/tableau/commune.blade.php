...
<div class="row">
    ...
    <div class="col-md-9">
        <div class="x_title">
                    <h2>Liste des membres dans la coordination du cercle de {{$nameCercle}}</h2>
    ...
    <div class="col-md-3">
         <div class="x_panel">
			...
			{{--   <h6>Nombre de coordination de cercle :</h6>
			 <p>{{$adhesion}}</p>
			  <hr>  --}}
			 <h6>Nombre de membre dans les coordinations de commune du cercle : {{$nameCercle}}</h6>
		 
             <hr>
             @foreach ($communes as $item)
                <div > <a style="display:flex; justify-content:space-between" href="#">{{$item->non}} <span>{{countInCommune($item)}}</span></a> </div>
                @endforeach
                <hr>
                <div style="display:flex; justify-content:space-between"> <a href="">Total</a> <span>{{$communeTotal}}</span> </div>
